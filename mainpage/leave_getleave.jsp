...
String EMP_ID=(String)session.getAttribute("input_user");
  ...
  try{
    ...
    String sql="SELECT * FROM FORM_HEAD_OF_PSEUDOMONAS WHERE EMP_ID = "+"'"+EMP_ID+"'"+"ORDER BY FORM_NO DESC";
...
<center>
<div class="span_title"><strong>假單狀況</strong></div>
  <H3>
    ...
    <table class="table table-striped" cellpadding="4" style="width: 750px;" >
      ...
      for(int i = 1; i <= PageSize; i++){
        ...
        <tr>
          ...
          <form action = "leave_getstatus.jsp" mothed="post">
            <td>
              <input type="hidden" name = "FORM_NO" value="<%=rs.getString("FORM_NO")%>"><button class="btn btn-default btn-sm" type="submit">詳細資訊</button>
              <%if(STATUS.equals("2")){ %>
              <input type="hidden" name = "FORM_NO_alter" value="<%=rs.getString("FORM_NO")%>">
              <button class="btn btn-default btn-sm" type="button" onClick="this.form.action='leave_alter.jsp';this.form.submit();">修改</button>
              <button class="btn btn-default btn-sm" type="button" onClick="this.form.action='leave_delect.jsp';this.form.submit();">刪除</button>
              <%}%>
            </td>
          </form>
        </tr>
...
<%
  ...
  if(ShowPage != 1)
  {
    ...
    %>
    | 
    <A Href=leave_getleave.jsp?ToPage=<%= 1 %>> <span class="glyphicon glyphicon-fast-backward" aria-hidden="true"></span>回第一頁 |</A>
    <A Href=leave_getleave.jsp?ToPage=<%= ShowPage - 1 %>> <span class="glyphicon glyphicon-step-backward" aria-hidden="true"></span>回上一頁 |</A>
    <%
  }
  ...
  //下面建立的各超連結將連結至自己，並將欲顯示的分頁以ToPage參數傳遞自己
    %>
    <A Href=leave_getleave.jsp?ToPage=<%= ShowPage + 1 %>> <span class="glyphicon glyphicon-step-forward" aria-hidden="true"></span>回下一頁 |</A>
    <A Href=leave_getleave.jsp?ToPage=<%= PageCount %>> <span class="glyphicon glyphicon-fast-forward" aria-hidden="true"></span>最後一頁 | </A>

    <%
  }
%>
  <form action="leave_getleave.jsp" method=POST>
  <!--
